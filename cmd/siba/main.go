...
func main() {
	...
	case "export":
		runExport(jsonMode)
	case "build":
		buildArgs := argsWithout(2, "--json")
		if len(buildArgs) == 0 {
			runBuild("", jsonMode)
		} else {
			runBuild(buildArgs[0], jsonMode)
		}
	case "check":
...
// runBuild renders every `*.siba.md` template (or a single one if path is set)
// and writes the result to the sibling `.md` file. Generated by siba so the
// output is autoloadable by Claude Code while the template stays free to use
// siba directives + imports + embeds.
func runBuild(target string, jsonMode bool) {
	cwd := mustGetwd()
	...
	for path, doc := range ws.DocsByPath {
		...
		if target != "" {
			abs, _ := filepath.Abs(filepath.Join(ws.Root, path))
			tgtAbs, _ := filepath.Abs(target)
			if abs != tgtAbs && path != target {
				continue
			}
		}
		...
		if werr := os.WriteFile(outFull, buf.Bytes(), 0o644); werr != nil {
			results = append(results, result{Source: path, Output: out, Error: werr.Error()})
			continue
		}
		results = append(results, result{Source: path, Output: out})
	}
	...
	if len(results) == 0 {
		fmt.Println("no .siba.md templates found")
		return
	}
	for _, r := range results {
		if r.Error != "" {
			fmt.Fprintf(os.Stderr, "\033[31m✗\033[0m %s → %s: %s\n", r.Source, r.Output, r.Error)
			continue
		}
...
func runHelp(topic string) {
	...
	case "directives":
		...
		fmt.Println("  @doc <name> extends <par>   Document inheriting from a template")
		fmt.Println("  @template <name>            Declare as template")
		fmt.Println("  @template <name> extends... Template inheriting another template")
		fmt.Println("  @extends <name>             (deprecated — use modifier above)")
	...
	case "templates":
		...
		fmt.Println("===============")
		fmt.Println()
		fmt.Println("  <!-- @template MyTemplate -->         Declare template")
		fmt.Println("  <!-- @doc MyDoc extends MyTemplate --> Use template (modifier)")
		fmt.Println("  <!-- @extends MyTemplate -->          (deprecated)")
		fmt.Println()
	...
	case "references":
		...
		fmt.Println()
		fmt.Println("  Import:")
		fmt.Println("    <!-- @import alias from ./path.md -->")
		fmt.Println()
		fmt.Println("  Local:")
		fmt.Println("    {{variable}}              Local variable (scope chain)")
		fmt.Println("    {{obj.prop}}              Object property")
		fmt.Println("    {{#section}}              Current file section/symbol")
		fmt.Println("    {{#parent/child}}         Nested symbol")
		fmt.Println()
		fmt.Println("  External (requires @import):")
		fmt.Println("    {{alias.variable}}        Module-level variable")
		fmt.Println("    {{alias#symbol}}          Symbol from imported file")
		fmt.Println("    {{alias#parent/child}}    Nested symbol from imported file")
		fmt.Println()
		fmt.Println("  Escape:")
		fmt.Println("    \\{{literal}}              Not expanded")
		fmt.Println()
		fmt.Println("  Note: external access requires @import. Direct {{doc-name.var}} is not supported.")
	case "control":
